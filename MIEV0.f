...
c ----------------------------------------------------------------------
      ...
      DOUBLE PRECISION GQSC,MIMCUT,PMOM( 0:MOMDIM, * ),QEXT,QSCA,SPIKE,
     &         XMU(*), XX
      DOUBLE COMPLEX CREFIN,SFORW,SBACK,S1(*),S2(*),TFORW(*),TBACK(*)
c ----------------------------------------------------------------------
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION CHIN,CHINM1,COEFF,DENAN,DENBN,FN,MIM,MM,MRE,
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DABS, DIMAG, DCMPLX, DCONJG, DCOS, MAX, MIN, DBLE, DSIN
c     ..
      SAVE      PASS1
...
c                                            ** small-particle limit
     ...
     &                S1, S2, TFORW, TBACK, LITA, LITB )
      ...
      IF( .NOT.PERFCT ) THEN
         ...
         IF( XX*MAX( 1.0, ZABS(CIOR) ).LE.0.1 ) THEN
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DABS, DIMAG, ICHAR, MAX, DBLE
c     ..
...
c     .. Local Arrays ..
      ...
      DOUBLE PRECISION AM( 0:MAXTRM ),BI( 0:MXMOM2 ),BIDEL( 0:MXMOM2 ),
     &          RECIP( MAXRCP )
      DOUBLE COMPLEX C( MAXTRM ),CM( MAXTRM ),CS( MAXTRM ),D( MAXTRM ),
     &          DM( MAXTRM ), DS( MAXTRM )
c     ..
c     .. External Subroutines ..
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DIMAG, DCONJG, MAX, MIN, MOD, DBLE
c     ..
c     .. Equivalences ..
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DIMAG, DCONJG, MIN, DBLE
c     ..
c     .. Statement Functions ..
...
c                               ** Loop over Mueller moments
     ...
     &                                          + 3.*CH ) / 12.
               ...
               IF( L.EQ.2 ) PMOM( L, 3 ) = 0.1 * DBLE( CG
     &                                      + (200./7.) * B2C * A(2) )
     ...
     &                                           + 3.*CH ) / 12.
               ...
               IF( L.EQ.2 ) PMOM( L, 4 ) = 0.1 * DIMAG( CG
     &                                     + (200./7.) * B2C * A(2) )
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DABS, DIMAG, DCOS, CDEXP, DBLE, DSIN
c     ..
c     .. Statement Functions ..
...
c                                 ** Eq. R25b
      CAK = ( MM*KK ) * ZINV
c                                          ** Eq. R32
      IF( ZABS( CNUMER / CAK ).LE.EPS1 .OR.
     &    ZABS( CDENOM / CAK ).LE.EPS1 ) THEN
...
c     .. Scalar Arguments ..
      ...
      DOUBLE PRECISION      GQSC, QEXT, QSCA, XX
      DOUBLE COMPLEX   CREFIN, SBACK, SFORW
c     ..
c     .. Array Arguments ..

      LOGICAL   CALCMO( * ), PRNT( * )
      DOUBLE PRECISION      PMOM( 0:MOMDIM, * ), XMU( * )
      DOUBLE COMPLEX   S1( * ), S2( * ), TBACK( * ), TFORW( * )
c     ..
c     .. Local Scalars ..
...
c     .. Scalar Arguments ..
      ...
      INTEGER   NUMANG
      DOUBLE PRECISION      GQSC, QEXT, QSCA, XX
      DOUBLE COMPLEX   SBACK, SFORW
c     ..
c     .. Array Arguments ..

      DOUBLE PRECISION      XMU( * )
      DOUBLE COMPLEX A( * ),B( * ),S1( * ),S2( * ),TBACK( * ),TFORW( * )
c     ..
c     .. Local Scalars ..
...
c     .. Intrinsic Functions ..
      ...
      INTRINSIC DIMAG, DCMPLX, DCONJG, DBLE
c     ..
c     .. Statement Functions ..
...
c                                                       ** Eq. R/A.5
      A( 1 ) = DCMPLX( 0., TWOTHR*( 1. - 0.2*XX**2 ) ) /
     &         DCMPLX( 1. - 0.5*XX**2, TWOTHR*XX**3 )
c                                                       ** Eq. R/A.6
      B( 1 ) = DCMPLX( 0., - ( 1. - 0.1*XX**2 ) / 3.) /
     &         DCMPLX( 1. + 0.5*XX**2, - XX**3 / 3.)
c                                                       ** Eq. R/A.7,8
      A( 2 ) = DCMPLX( 0.,   XX**2 / 30.)
      B( 2 ) = DCMPLX( 0., - XX**2 / 45.)
c                                                       ** Eq. R/A.9
...
c     .. Scalar Arguments ..
      ...
      INTEGER   NUMANG
      DOUBLE PRECISION      GQSC, QEXT, QSCA, XX
      DOUBLE COMPLEX   CIOR, SBACK, SFORW
c     ..
c     .. Array Arguments ..
...
c     ..
      ...
      CIORSQ = CIOR**2
      CTMP   = DCMPLX( 0., TWOTHR )*( CIORSQ - 1.0 )
...
c     .. Scalar Arguments ..
      ...
      INTEGER   IPOLZN, MOMDIM, NMOM, NUMANG
      DOUBLE PRECISION      GQSC, MIMCUT, QEXT, QSCA, XX
      DOUBLE COMPLEX   CREFIN, SBACK, SFORW
c     ..
c     .. Array Arguments ..
...
c     .. Local Scalars ..
      ...
      DOUBLE PRECISION ACCUR,CALC,EXACT,MIMSAV,TESTGQ,TESTQE,TESTQS,
     &          XMUSAV, XXSAV
      DOUBLE COMPLEX   CRESAV, TESTS1, TESTS2, TESTSB, TESTSF
c     ..
c     .. Local Arrays ..

      LOGICAL   CALCMO( 4 ), PRNT( 2 )
      DOUBLE PRECISION      TESTPM( 0:1 )
      DOUBLE COMPLEX   TESTTB( 2 ), TESTTF( 2 )
c     ..
c     .. External Functions ..
...
c                                    ** correct answers and abort if bad
   ...
   10    CONTINUE
     ...
     &           OK =  TSTBAD( 'PMOM', DABS( (PMOM(M,1)-TESTPM(M)) /
     &                                      TESTPM(M) ) )
